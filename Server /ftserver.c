...
int main (int argc, char *argv[]) {
	...
	while(1) {
		...
		if(pid == 0)
		{
	    	...
	    	//printf("Recieved Data port form client : %s\n",dataPort );
	    	n = write(newsockfd,"ok",3);
			// wipe out the content of hostName
			bzero(hostName,50);
	    	n = read(newsockfd,hostName,10);
	    	//printf("Recieved Hostname form client : %s\n",hostName );
	    	n = write(newsockfd,"ok",3);
	    	// wipe out the content of dataPort
			bzero(command,50);
	    	n = read(newsockfd,command,5);
	    	//printf("Recieved command form client : %s\n",command );
	    	if (strcmp(command,"-g") == 0) {
		    	...
		    	//printf("New Recieved fileNameLength form client : %s\n",fileNameLength );
		    	n = write(newsockfd,"ok",3);
		    	size = atoi(fileNameLength);  	
		    	n = read(newsockfd,fileName,size + 1);
		    	//printf("Recieved fileName form client : %s\n",fileName );
	    	}
			...
			if (strcmp(Buffer,"READY") == 0) {
				...
				if (strcmp(command,"-g") == 0 || strcmp(command,"-l") == 0) {
		    		...
		    		//n = write(dataSockfd,"TCP Data Socket Started",50);
